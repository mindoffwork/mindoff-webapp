    ...
    <>
      <section className="relative border-y-black border-y-2px md:border-y-3px">
        <nav aria-label="Topics Navigation">
          <ul className="flex overflow-x-auto scrollable overscroll-x-none flex-row list-none px-0 md:px-6 lg:px-8 " id="topics_slider__id">
            {topicsList.map((topic) => (
              ...
              >
                ...
                >
                  <div>
                    <span className="material-symbols-outlined w-5 h-5 lg:w-6 lg:h-6" aria-hidden="true">
                      {topic.icon}
                    </span>
                  </div>
                  <div>
                    <h2 className="mt-2 font-bold tracking-wide uppercase text-1xs lg:text-xs">{topic.name}</h2>
                  </div>
                </Link>
              </li>
            ))}
