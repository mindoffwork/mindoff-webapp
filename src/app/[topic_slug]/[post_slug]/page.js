...
export default async function PostPage(post_slug) {
  ...
  const [postHead, postBody] = await Promise.all([
    ApiCaller("GetPostHead" , currentSlug, null),
    ApiCaller("GetPostBody" , currentSlug, null)
  ]);
  // 3. Handle -- Errors
          ...
          <section className="lg:w-8/12">
            ...
            <section className="w-10/12 mx-auto py-16 content" aria-label="Post Content">
              {postBody.map((block, index) =>
                block.name === "text" ? (
                  <BlockText key={block.name + index} name={block.name} content={block.content} />
                ) : block.name === "img" ? (
                  <BlockImage key={block.name + index} name={block.name} content={block.content} link={block.link} />
                ) : block.name === "script" ? (
                  <BlockScript key={block.name + index} name={block.name} content={block.content} link={block.link} />
                ) : null
              )}
            </section>
          </section>
          ...
          <section>
            ...
            <div className="-mt-2px md:-mt-3px">
              <Suspense fallback={<CardsFourSkeleton />}>
                <CardsFour apiName={"GetPostsTopicwise"} formatName={postHead.topic}/>
              </Suspense>
            </div>
          </section>
          <section>
            ...
            <div className="-mt-2px md:-mt-3px">
              <Suspense fallback={<CardsThreeSkeleton />}>
                <CardsThree apiName={"GetPostsExpo"} formatName={"featured"}/>
              </Suspense>
            </div>
          </section>
        </aside>
